...
export function StudentUnitOverview({
  ...
  return <div className="grid gap-6">
    {startingPointComplete ? <p className="text-sm text-slate-600">Starting point complete. Your original result is saved.</p> : <section className="card border-blue-200 bg-blue-50">
    ...
    {access&&startingPointComplete&&access.focus.week>1&&focusTopic&&<section className="card border-blue-200 bg-blue-50">
      <p className="eyebrow">Your next step · Week {access.focus.week}</p>
      ...
      {(access.currentWeekBlocked||unfinishedStartedWeek)&&<p className="mt-3 text-slate-700">Your place is saved. Continue one step at a time.</p>}
    ...
    {access?.currentWeekBlocked&&<p className="text-sm text-slate-600">Finish this step before the next week opens.</p>}
