...
export default async function MiniStudyGroupPage({params}:{params:Promise<{id:string}>}){
 ...
 const reportUnits:StudyUnitRef[]=(activeUnits??[]).map(item=>{const unit=Array.isArray(item.units)?item.units[0]:item.units;return {id:item.unit_id,label:unit?`Unit ${unit.code}: ${unit.title}`:"Unit details unavailable"};});
 const evidence=!unitsError?await loadMiniStudyEvidence(client,id,activeUnitIds).catch(()=>null):null;
 const reminders=await practiceReminders(id).catch(()=>null);
 ...
 return <><AppHeader name={actor.display_name} role={actor.role}/><main className="shell max-w-5xl py-8">
  ...
  {reminders===null&&<p role="status">Practice reminder status could not be loaded.</p>}
  <p className="eyebrow mt-6">{group.name}</p>
  {unitNames.length>0&&<p className="mt-2 text-slate-600">{unitNames.join(" · ")}</p>}
  {!activeUnitIds.length||!group.published?<section className="card mt-5"><h1 className="text-3xl font-bold">Finish this group’s setup</h1><p className="mt-3">Choose the units you teach before students begin.</p><Link className="button mt-5" href={`/teacher/classes/${id}/settings`}>Set up group →</Link></section>:<div className="mt-5">{!evidence?<section className="card" role="alert"><h1 className="text-2xl font-bold">Records could not be loaded</h1><p className="mt-3">Refresh to try again. Missing evidence has not been shown as zero progress.</p></section>:<><MiniStudyReport overdueLearnerIds={reminders?.map(r=>r.learnerId)??[]} {...evidence} units={reportUnits} classId={id}/><a className="button-secondary mt-6" href={`/api/reports/classes/${id}/mini-study`}>Download short-study spreadsheet</a></>}</div>}
