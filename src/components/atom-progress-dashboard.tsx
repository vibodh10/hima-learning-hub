...
export function AtomProgressDashboard({units,initialAttempts=[],storageKey}:{units:PearsonUnit[];initialAttempts?:AtomAttemptSummary[];storageKey:string}){
 const[progress,setProgress]=useState<LearningProgress>({topics:{}}),[loaded,setLoaded]=useState(false);
 useEffect(()=>{let active=true;queueMicrotask(()=>{if(!active)return;try{setProgress(JSON.parse(localStorage.getItem(storageKey)??"") as LearningProgress)}catch{}setLoaded(true)});return()=>{active=false}},[storageKey]);
 const rows=units.flatMap(unit=>unit.topics.map(topic=>{const evidence=progress.topics[topicKey(unit.code,topic.code)]??{},server=initialAttempts.find(item=>item.kind==="topic_practice"&&item.unitCode===unit.code&&item.topicCode===topic.code);return{unit,topic,evidence:{...evidence,...(server?{practiceScore:server.percentage,hintsUsed:server.hintsUsed}:{})}}}));
 ...
 return <div className="grid gap-7">
  ...
  <section className="card"><p className="eyebrow">Paper history</p><h2 className="mt-2 text-2xl font-bold">Your recent practice papers</h2><div className="mt-5 grid gap-3">{papers.length?papers.slice(0,8).map(paper=><div className="rounded-xl bg-slate-50 p-4" key={paper.id}><div className="flex flex-wrap justify-between gap-3"><span><strong>Unit {paper.unitCode}</strong> · {paper.paperMode??"applied"} · {new Date(paper.completedAt).toLocaleDateString("en-GB")}</span><span className="font-bold">{paper.hasSubmittedResponses&&paper.teacherMark==null?"Awaiting Hima review":`${paper.percentage}% · ${paper.mark}/${paper.maxMark} marks`} · {Math.round(paper.activeSeconds/60)} min</span></div>{paper.teacherFeedback&&<p className="mt-3 border-t border-teal-200 pt-3 text-sm text-teal-950"><strong>Hima’s feedback:</strong> {paper.teacherFeedback}</p>}</div>):<p className="text-slate-600">No signed-in practice-paper attempts recorded yet.</p>}</div></section>
 </div>;
}
