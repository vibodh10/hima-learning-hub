...
export function RewardPurchaseForm({
  ...
  const purchaseResult=state.testData&&typeof state.testData==="object"
    ?state.testData as {purchaseId?:string;kind?:string;equipped?:boolean;assetConfig?:Record<string,unknown>}:undefined;
  ...
  useEffect(()=>{
    if(!state.ok||!purchaseResult?.kind||purchaseResult.equipped!==true)return;
    applyEquippedCosmetic(purchaseResult.kind,true,purchaseResult.assetConfig??{});
  },[state.ok,purchaseResult]);
  ...
  return <div className="mt-4">
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {currentOwned&&currentPurchaseId&&<form action={equipAction}><input type="hidden" name="purchaseId" value={currentPurchaseId}/><input type="hidden" name="equip" value={currentEquipped?"false":"true"}/><button className="button" disabled={equipPending}>{equipPending?"Saving…":currentEquipped?"Unequip":"Equip"}</button></form>}
      {currentEquipped&&<span className="rounded-full bg-teal-100 px-3 py-2 text-sm font-bold text-teal-900">Applied</span>}
    </div>
