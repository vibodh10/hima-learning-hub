...
export function StartingPointAssessment({ unit, storageKey }: { unit: PearsonUnit; storageKey: string }) {
  ...
  if (!started) {
    return <div className="grid gap-6">
      ...
      <section className="card"><h2 className="text-2xl font-bold">Ready to begin?</h2><p className="mt-2 text-slate-600">You will see one question at a time. Your answers are saved only when you finish.</p><details className="mt-5 rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer font-semibold">Add optional background information</summary><p className="mt-2 text-sm text-slate-600">These answers help staff understand you but never count as mastery.</p><label className="mt-4 block font-semibold">Previous experience<textarea className="input mt-2 min-h-24" value={experience} onChange={event=>setExperience(event.target.value)}/></label><label className="mt-4 block font-semibold">Support or accessibility needs<textarea className="input mt-2 min-h-24" value={supportNeeds} onChange={event=>setSupportNeeds(event.target.value)}/></label></details><button className="button mt-5" type="button" onClick={()=>setStarted(true)}>Start question 1 →</button></section>
    </div>;
  }
