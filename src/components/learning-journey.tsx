...
export function UnitOverview({ unit, initialProgress = null, storageKey = progressKey }: { unit: PearsonUnit; initialProgress?: LearningProgress | null; storageKey?: string }) {
  ...
  return <div>
    ...
    <section className="mt-8">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">Topics</p><h2 className="mt-2 text-3xl font-bold">Choose what to learn</h2></div><span className="text-sm text-slate-500">{unit.topics.length} complete lessons</span></div>
      <div className="mt-5 grid gap-4 md:grid-cols-2">{unit.topics.map((topic, index) => {
        ...
        return <Link href={topicHref(unit.code, topic.code)} className="card group hover:border-teal-400 hover:bg-teal-50" key={topic.code}>
          <p className="text-xs font-bold uppercase tracking-wide text-teal-700">Topic {index + 1} · {topic.code}</p>
