...
export function AcademicCalendarForm({
  ...
  return <form action={action} className="grid gap-3 rounded-xl border border-slate-200 p-4">
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm font-semibold">Event type<select className="input" name="kind" defaultValue={event?.kind??"holiday"}>{[["holiday","Holiday / half term"],["college_closure","College closure day"],["non_teaching","Other non-teaching period"],["teaching_week","Teaching week"],["progress_point_week","Progress-point week"],["review_week","Review week"],["examination_reminder","Examination reminder"]].map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label>
