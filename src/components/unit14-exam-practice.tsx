...
import {unit14Activities,unit14Papers,unit14Catalogue} from "@/lib/unit14-exam";
...
function PracticeAttempt({classId,staff,restart}:{classId:string;staff:boolean;restart:()=>void}){
 ...
 {paper==="guided"?<><h3 className="text-xl font-bold">One strong approach</h3><p>This is an original teaching example, not an official Pearson answer or a guaranteed full-mark response. Other justified designs can work.</p><p>{model.model}</p><h4 className="font-bold">Compare your answer</h4><ul className="list-disc pl-6">{model.checks.map(c=><li key={c}>{c}</li>)}</ul></>:<><h3 className="text-xl font-bold">Review against Pearson&apos;s guidance</h3><p>Use the matching marking grid and examiner examples. There is no single correct design. Identify evidence in your own work before deciding what to improve.</p>{chosen?.links.filter(l=>l.title.includes("scheme")||l.title.includes("report")||l.title.includes("marking")).map(l=><p key={l.url}><a className="link" href={l.url} target="_blank" rel="noreferrer">{l.title}</a></p>)}<p><a className="link" href={unit14Catalogue} target="_blank" rel="noreferrer">Pearson Unit 14 marking guidance and reports</a></p><p>If a matching grid or example is absent, ask your teacher for it. Do not treat an answer from a different scenario as the model for this task.</p></>}
