...
export function TeacherAccountSetupForm({existingAccounts=[]}:{existingAccounts?:ExistingTeacherAccount[]}) {
  ...
  return <section className="card mt-6 border-teal-200" aria-labelledby="teacher-account-title">
    ...
    <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Choose a tutor and enter their verified staff email, then create a registration link to share privately in Teams. No email delivery is needed. The tutor chooses their own password. This also works for an existing teacher who cannot sign in.</p>
    ...
    <form action={action} className="mt-5 grid gap-4 md:grid-cols-[1fr_1.2fr_auto] md:items-end">
      ...
      <div className="flex flex-wrap gap-2"><button className="button" name="delivery" value="manual" disabled={pending}>{pending ? "Working…" : "Create registration link"}</button><button className="button-secondary" name="delivery" value="email" disabled={pending}>{pending ? "Working…" : "Email setup link"}</button></div>
