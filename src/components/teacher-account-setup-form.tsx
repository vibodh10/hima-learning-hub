...
type ExistingTeacherAccount={name:string;email:string|null};
      ...
      {requestedTeacherNames.map(name=>{
        ...
        return <article className="rounded-xl border border-slate-200 p-4" key={name}>
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${account?"bg-emerald-100 text-emerald-950":"bg-amber-100 text-amber-950"}`}>{account?"Access active":"Login needed"}</span>
          </div>
          <p className="mt-2 break-all text-sm text-slate-600">{account?.email??"Enter this tutor's verified email below."}</p>
        </article>;
      })}
    </div>
