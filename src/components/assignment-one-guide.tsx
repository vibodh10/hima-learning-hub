...
export function AssignmentOneGuide({schedule,preview=false}:{schedule:ReturnType<typeof assignmentSchedule>;preview?:boolean}) {
  const [index,setIndex]=useState(schedule.current);
  const [answer,setAnswer]=useState<number|null>(null);
  const [checked,setChecked]=useState(false);
  const title=useRef<HTMLHeadingElement>(null);
  const step=assignmentSteps[index];
  ...
  return <div className="mini-study-surface">
    ...
    <main id="assignment-main" className="mini-study-main">
      ...
      <p className="assignment-note">Unit 6 Website Development · Learning aim A. Use this preparation guide alongside your teacher&apos;s assignment brief.</p>
      <details className="mini-study-help assignment-plan"><summary>See the plan or choose a step</summary>
        ...
        <select id="assignment-step" value={index} onChange={event=>go(Number(event.target.value))}>
          {assignmentSteps.map((item,i)=><option key={item.id} value={i}>{i+1}. {item.title} ({item.dates})</option>)}
        </select>
      ...
      <section className="mini-study-panel" aria-labelledby="assignment-title">
        <p className="mini-study-kicker">Step {index+1} of {assignmentSteps.length} · Suggested {step.dates}</p>
        ...
        <nav className="assignment-navigation" aria-label="Preparation steps">
          ...
          {index<assignmentSteps.length-1?<button className="mini-study-primary" type="button" onClick={()=>go(index+1)}>Next step</button>:<Link href={back} className="mini-study-primary">Back to learning</Link>}
        </nav>
      </section>
      ...
      <details className="mini-study-help"><summary>About the requirements</summary><p>This guide supports comparison (A.P1), analysis (A.M1) and evaluation (A.D1). It does not replace your brief or cover the whole of learning aims B and C. Your assessor judges your independent work.</p><p>Harvard is taught here as a consistent author–date format. The specification does not prescribe a Harvard variant or require the particular testing tools linked above.</p><a className="link" href={`${assignmentOne.specification}#page=79`} target="_blank" rel="noreferrer">Pearson Diploma specification: Unit 6 (opens in a new tab)</a></details>
    </main>
    <footer className="mini-study-footer"><Link href={back}>Back to your learning</Link><a href="/assignment-one-evidence-template.txt" download>Download a blank evidence sheet</a></footer>
  </div>;
}
