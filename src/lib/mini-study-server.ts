...
async function studyHomeForContext(learnerId:string,context:StudyContext|null,continueToday=false):Promise<StudyHome> {
  ...
  return hasStep
    ...
    : {status:"complete",message:"You are up to date with the topics your tutor has taught so far. The Digital Learning Hub will wait for the next formal check or give second practice automatically when evidence shows a skill needs reinforcement."};
}
...
export async function openStudySession(learnerId:string,continueToday=false):Promise<StudyHome> {
  ...
  if(!selected&&!assessment&&home.kind==="daily") return {status:"complete",message:"You are up to date with the topics your tutor has taught so far. The Digital Learning Hub will wait for the next formal check or give second practice automatically when evidence shows a skill needs reinforcement."};
  ...
  const card:Omit<StudyCard,"sessionId"|"questions">=home.kind==="baseline"
    ...
    : assessment
      ? {kind:"daily",title:assessment.title,unitTitle:context.unitTitle,assessmentKind:assessment.kind,assessmentNumber:assessment.number,
          lines:["This formal learning check covers material your tutor has already taught in class.","Answer independently. If a skill is not secure, the Digital Learning Hub will automatically give you relevant second practice and recheck it."],example:"",support:""}
