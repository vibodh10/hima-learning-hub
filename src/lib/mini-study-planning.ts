...
export type StudyHistoryRow={class_id:string;unit_id:string;status:string;kind:"baseline"|"daily";lesson_id:string;grade:StudyGrade|null;completed_at:string|null;opened_at?:string;paused_for_starting_point?:boolean};
...
export function selectStudyAssignment(assignments:StudyAssignment[],history:StudyHistoryRow[],hasNextStep:(assignment:StudyAssignment)=>boolean=()=>true):StudyAssignment|undefined {
  const active=history.find(row=>!row.paused_for_starting_point&&["opened","review"].includes(row.status)&&assignments.some(a=>a.classId===row.class_id&&a.unitId===row.unit_id));
  ...
  return [...(available.length?available:assignments)].sort((a,b)=>last(a).localeCompare(last(b))||a.classId.localeCompare(b.classId)||a.unitId.localeCompare(b.unitId))[0];
}
