...
async function StudentDashboard({ id, name }: { id: string; name: string }) {
  ...
  return <main className="shell py-10">
    ...
    <section className={`card mt-6 ${catchUps.some(item=>item.status!=="completed")?"border-amber-200":""}`} aria-labelledby="catch-up-title"><p className="eyebrow">Missed learning</p><h2 className="mt-2 text-2xl font-bold" id="catch-up-title">Catch-up</h2><div className="mt-5 grid gap-3">{catchUps.some(item=>item.status!=="completed")?catchUps.filter(item=>item.status!=="completed").map(item=>{const topic=topicByCode(item.unit_code,item.topic_code);return <Link className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-4 hover:bg-amber-50" href={`/curriculum/units/${item.unit_code}/topics/${encodeURIComponent(item.topic_code)}?catchup=1#worksheet`} key={item.catch_up_id}><div><p className="font-semibold">Unit {item.unit_code} · {topic?.title??item.topic_code}</p><p className="mt-1 text-sm text-slate-600">Opened in Teaching Week {item.opened_teaching_week} · now Week {item.current_teaching_week}</p></div><span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-bold text-amber-950">{item.status.replaceAll("_"," ")}</span></Link>}):<p className="rounded-xl bg-emerald-50 p-4 text-sm text-emerald-950"><strong>Catch-up required: 0.</strong> You have no recorded missed learning to complete.</p>}</div></section>
