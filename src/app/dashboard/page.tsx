...
import {CurrentStudyOverview} from "@/components/current-study-overview";
...
async function AdministratorDashboard({role, filters}: { role: "administrator"; filters: TeacherFilters }) {
    ...
    const classSignals = await Promise.all((classes ?? []).map(async item => {
        ...
        const attention = ((attentionData ?? []) as TeacherAttentionDb[]).map(row => ({
            ...row, classId: item.id, className: item.name,
    ...
    const completionEvidence = (allocationEvidence ?? []).flatMap(allocation => {
        ...
        return learners.map(learnerId => {
            ...
            return {
                classId: allocation.class_id, learnerId, activityId: allocation.activity_id, status,
    ...
    const attention = allAttention.filter(item => visibleClassIds.has(item.classId) && selectedLearners.has(item.learner_id)).map(item => {
        ...
        const scoped = scopedTeacherAttention({
            baseStatus: item.attention_status,
            baseReason: item.attention_reason,
            catchUpStatus: item.catch_up_status,
            outstandingCount: item.outstanding_count,
            currentScore
        });
        return {
            ...item,
            current_score: currentScore,
            progress_points: scopedProgressByLearner.get(item.learner_id) ?? null,
            attention_status: scoped.status,
            attention_reason: scoped.reason
        };
    ...
    return <main className="shell py-10">
        ...
        <div className="mt-8 flex flex-wrap items-end justify-between gap-5">
            <div><p className="eyebrow">{role === "administrator" ? "Teaching administration" : "Teacher home"}</p><h1
                className="mt-2 text-4xl font-bold">{role === "administrator" ? "Groups and progress" : "Your groups"}</h1>
                <p className="mt-2 max-w-3xl text-slate-600">Open a group, see its students and progress, then download
                    a report when you need one.</p></div>
            {role === "administrator" && <Link className="button-secondary" href="/admin">Administration</Link>}</div>
        <CurrentStudyOverview groups={visibleClasses}/>
        <details className="card mt-6">
            <summary className="cursor-pointer text-lg font-bold">Earlier task alerts and setup suggestions</summary>
            <p className="mt-3 text-slate-600">These alerts come from earlier targets and task allocations. Completing a
                new short lesson does not close an older target. Review the original reason before deciding whether
                action is still needed.</p>
            <section className="mt-5" aria-labelledby="teacher-next-action-title">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="max-w-3xl"><p className="eyebrow">{teacherNextAction.eyebrow}</p><h2
                        className="mt-2 text-3xl font-bold"
                        id="teacher-next-action-title">{teacherNextAction.title}</h2><p
                        className="mt-3 leading-7 text-slate-700">{teacherNextAction.detail}</p></div>
                    {teacherNextAction.meta && <span
                        className="rounded-full bg-white px-3 py-2 text-sm font-bold text-slate-900">{teacherNextAction.meta}</span>}
                </div>
                <Link className="button mt-6 min-w-40 text-center"
                      href={teacherNextAction.href}>{teacherNextAction.label} â†’</Link>
            </section>
        </details>
        <section className="card mt-6" id="groups" aria-labelledby="groups-title">
            <div><p className="eyebrow">My groups</p><h2 className="mt-2 text-2xl font-bold" id="groups-title">Choose a
                group</h2><p className="mt-2 text-sm text-slate-600">Open a group to see its students, progress and
                report. Unit setup is shown only to administrators.</p></div>
            ...
            {role === "administrator" && <details className="mt-5 rounded-xl border border-slate-200 p-4">
                <summary className="cursor-pointer font-bold">Add a group</summary>
                <p className="mt-2 text-sm text-slate-600">Administrator setup only. Teachers receive ready-to-use
                    groups.</p><CreateClassForm courses={courses ?? []} years={years ?? []}/></details>}
        </section>
        <details className="card mt-6">
            <summary className="cursor-pointer text-lg font-bold">Earlier assessments, targets and detailed reports
            </summary>
            <p className="mt-2 text-sm text-slate-600">These totals and alerts use the earlier assessment and target
                system. They do not count the new short lessons shown above. Older alerts are retained for review and do
                not establish that current lessons were missed.</p>
            <section className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4" aria-label="Earlier workflow totals">
                <Metric label="Students" value={String(overview.students)}/><Metric label="Active enrolments"
                                                                                    value={String(overview.activeEnrolments)}/><Metric
                label="Earlier assessments completed" value={String(overview.completedAssessments)}/><Metric
                label="Earlier group/student alerts" value={String(overview.needAttention)}/></section>

            {totalStudentCount > 0 && <section className="card mt-6 overflow-x-auto">
                <div><p className="eyebrow">Priority list</p><h2 className="mt-2 text-2xl font-bold">Learner updates</h2>
                    <p className="mt-2 text-sm text-slate-600">Earlier recorded targets, catch-up and learning evidence. Informational notices do not assign you a task. Check the current short-study record before deciding whether support is needed.</p></div>
                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><PriorityCount
                    label="Intervention Required"
                    value={attention.filter(item => item.attention_status === "intervention_required").length}
                    status="intervention_required"/><PriorityCount label="For your information"
                                                                   value={attention.filter(item => item.attention_status === "action_required").length}
                                                                   status="action_required"/><PriorityCount
                    label="Catch-up Required"
                    value={attention.filter(item => item.attention_status === "catch_up_required").length}
                    status="catch_up_required"/><PriorityCount label="On Track"
                                                               value={attention.filter(item => item.attention_status === "on_track").length}
                                                               status="on_track"/><PriorityCount label="Exceeding"
                                                                                                 value={attention.filter(item => item.attention_status === "exceeding").length}
                                                                                                 status="exceeding"/>
                </div>
                <table className="mt-6 w-full min-w-[1050px] text-left text-sm">
                    <thead>
                    <tr className="border-b border-slate-200 text-slate-600">
                        ...
                        <th className="pb-3">Catch-up / outstanding</th>
                        <th className="pb-3">Status and reason</th>
                    </tr>
                    </thead>
                    <tbody>{attention.slice(0, 20).map(item => <tr className="border-b border-slate-100"
                                                                   key={`${item.classId}:${item.learner_id}`}>
                        <td className="py-4 font-semibold"><Link className="link"
                                                                 href={`/teacher/learners/${item.learner_id}?classId=${item.classId}`}>{item.display_name}</Link>
                        </td>
                        <td>{item.className}</td>
                        <td>{item.current_score == null ? "Not recorded" : `${item.current_score}%`}</td>
                        <td>{item.progress_points == null ? "Not comparable" : `${Number(item.progress_points) >= 0 ? "+" : ""}${item.progress_points} pp`}</td>
                        <td><strong>{item.ap_total} AP Â· {item.achievement_level ?? "Building"}</strong><p
                            className="mt-1 text-xs text-slate-500">{item.next_level ? `${item.points_to_next} AP to ${item.next_level}` : "Highest configured level"}{item.certificate_status ? " Â· certificate review eligible" : ""}</p>
                        </td>
                        <td className="capitalize">{item.catch_up_status.replaceAll("_", " ")}{item.outstanding_count ? ` Â· ${item.outstanding_count} outstanding` : ""}</td>
                        <td><PriorityBadge status={item.attention_status}/><p
                            className="mt-1 max-w-xs text-slate-500">{item.attention_reason}</p></td>
                    </tr>)}</tbody>
                </table>
                {!attention.length &&
                    <p className="mt-5 rounded-xl bg-slate-50 p-5 text-slate-600">No students match the current
                        filters. <Link className="link" href="/dashboard">Clear filters</Link></p>}</section>}

            {totalStudentCount > 0 &&
                <section className="card mt-6"><p className="eyebrow">Teaching-sequence milestones</p><h2
                    className="mt-2 text-2xl font-bold">Upcoming progress checks</h2><p
                    className="mt-2 text-sm text-slate-600">Derived from each group&apos;s teaching-week clock. College
                    holidays and closures do not consume a teaching week.</p>
                    <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{upcomingCheckpoints.map(item =>
                        <article className="rounded-xl border border-slate-200 p-4" key={item.classId}>
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                <div><h3 className="font-bold">{item.className}</h3><p
                                    className="mt-1 text-sm text-slate-600">Unit {item.unitCode} Â· now Teaching
                                    Week {item.teachingWeek}</p></div>
                                <span
                                    className="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-900">{journeyMilestoneLabel(item.milestone!.milestone)}</span>
                            </div>
                            <p className="mt-3 text-sm"><strong>Teaching
                                Week {item.milestone!.week}:</strong> {item.milestone!.title}
                            </p>{item.positionStatus === "paused" &&
                            <p className="mt-3 rounded-lg bg-sky-50 p-3 text-xs text-sky-950">Timer paused for a
                                non-teaching period; resumes {formatJourneyDate(item.nextTeachingOn)}.</p>}<Link
                            className="link mt-3 inline-block text-sm" href={`/teacher/classes/${item.classId}`}>Open
                            group evidence â†’</Link></article>)}{!upcomingCheckpoints.length &&
                        <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">No active group journey has a
                            further checkpoint in the current filters.</p>}</div>
                </section>}

            {totalStudentCount > 0 && <section className="card mt-6"><p className="eyebrow">Evidence signals</p><h2
                className="mt-2 text-2xl font-bold">Completion, progress and action</h2>
                <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Signal label="Overdue work"
                            value={filteredCompletionEvidence.filter(row => evidenceLearners.has(row.learnerId) && row.status === "overdue").length}
                            tone="risk"/>
                    <Signal label="Significant improvement"
                            value={improvementRows.filter(row => Number(row.improvement_points) >= 10).length}
                            tone="good"/>
                    <Signal label="No clear improvement"
                            value={improvementRows.filter(row => Number(row.improvement_points) <= 0).length}
                            tone="risk"/>
                    <Signal label="Learners ready for Stretch"
                            value={new Set(filteredMastery.filter(row => row.current_pathway === "Stretch").map(row => row.learner_id)).size}/>
                    <Signal label="Learners ready for Mastery"
                            value={new Set(filteredMastery.filter(row => row.current_pathway === "Mastery").map(row => row.learner_id)).size}/>
                    ...
                    <Signal label="Targets due"
                            value={activeTargets.filter(row => ["approved", "active", "extended"].includes(row.status) && row.target_date >= todayIso).length}/>
                    <Signal label="Targets achieved"
                            value={activeTargets.filter(row => row.status === "achieved").length} tone="good"/>
                    <Signal label="Overdue targets"
                            value={activeTargets.filter(row => ["approved", "active", "extended"].includes(row.status) && row.target_date < todayIso).length}
                            tone="risk"/>
                    <Signal label="Actions awaiting review" value={actionsAwaitingReview} tone="risk"/>
                </div>
                <p className="mt-4 text-sm text-slate-500">Counts are evidence records or distinct learners, not a
                    single overall average. Assessment readiness requires at least 70% recorded mastery across every
                    mapped skill in an active unit; it does not represent an official Pearson outcome. Open a class to
                    drill down through learner and attempt history.</p></section>}

            {totalStudentCount > 0 && <details className="card mt-6">
                <summary className="cursor-pointer text-lg font-bold">Filter this dashboard</summary>
                <form className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4" method="get">
                    <FilterSelect label="Academic year" name="academicYear" value={filters.academicYear}
                                  options={(years ?? []).map(item => ({id: item.id, title: item.name}))}/>
                    <FilterSelect label="Term / semester" name="period" value={filters.period}
                                  options={(periods ?? []).map(item => ({id: item.id, title: item.name}))}/>
                    <FilterSelect label="Course" name="course" value={filters.course}
                                  options={(courses ?? []).map(item => ({id: item.id, title: item.title}))}/>
                    <FilterSelect label="Class" name="class" value={filters.class}
                                  options={(classes ?? []).map(item => ({id: item.id, title: item.name}))}/>
                    <FilterSelect label="Student" name="student" value={filters.student}
                                  options={uniqueLearnerOptions(allAttention)}/>
                    <FilterSelect label="Unit / Content Area" name="unit" value={filters.unit}
                                  options={(units ?? []).map(item => ({
                                      id: item.id,
                                      title: `${item.code} Â· ${capitaliseFirst(item.title)}`
                                  }))}/>
                    <FilterSelect label="Topic" name="topic" value={filters.topic}
                                  options={(topics ?? []).map(item => ({
                                      id: item.id,
                                      title: capitaliseFirst(item.title)
                                  }))}/>
                    <FilterSelect label="Skill" name="skill" value={filters.skill}
                                  options={(skills ?? []).map(item => ({
                                      id: item.id,
                                      title: capitaliseFirst(item.title)
                                  }))}/>
                    <FilterSelect label="Pathway" name="pathway" value={filters.pathway}
                                  options={["Support", "Core", "Stretch", "Mastery"].map(item => ({
                                      id: item,
                                      title: item
                                  }))}/>
                    <FilterSelect label="Activity type" name="activityType" value={filters.activityType}
                                  options={[["in_class_learning", "Classroom learning"], ["in_class_practice", "Classwork"], ["homework", "Homework"], ["revision", "Revision"], ["holiday_work", "Holiday work"], ["skills_practice", "Practical skills"], ["review_check", "Review / progress check"]].map(([id, title]) => ({
                                      id,
                                      title
                                  }))}/>
                    <label className="grid gap-1 text-sm font-semibold">From date<input className="input" type="date"
                                                                                        name="dateFrom"
                                                                                        defaultValue={filters.dateFrom ?? ""}/></label>
                    <label className="grid gap-1 text-sm font-semibold">To date<input className="input" type="date"
                                                                                      name="dateTo"
                                                                                      defaultValue={filters.dateTo ?? ""}/></label>
                    <FilterSelect label="Completion status" name="completionStatus" value={filters.completionStatus}
                                  options={["assigned", "started", "completed", "overdue", "late", "not_attempted"].map(item => ({
                                      id: item,
                                      title: item.replaceAll("_", " ")
                                  }))}/>
                    <div className="flex items-end gap-3">
                        <button className="button-secondary">Apply filters</button>
                        <Link className="link pb-3 text-sm" href="/dashboard">Clear</Link></div>
                    <p className="text-xs text-slate-500 sm:col-span-2 lg:col-span-4">{filteredAttempts.filter(item => selectedLearners.has(item.learner_id)).length} completed
                        attempts match the activity/date filters
                        Â· {filteredCompletionEvidence.filter(item => selectedLearners.has(item.learnerId)).length} allocations
                        match the completion filter Â· {selectedLearners.size} learners match all active filters.</p>
                </form>
            </details>}

            {totalStudentCount > 0 && <section className="mt-6 grid gap-6 lg:grid-cols-2">
                <div className="card"><h2 className="text-xl font-bold">Common misconceptions</h2>
                    <div
                        className="mt-4 grid gap-3">{filteredMisconceptions.length ? filteredMisconceptions.slice(0, 5).map((row, index) =>
                            <div key={index} className="rounded-xl bg-amber-50 p-4"><p
                                className="font-semibold">{capitaliseFirst(related(row.misconceptions)?.title ?? "Recorded misconception")}</p>
                                <p className="mt-1 text-sm text-amber-900">{capitaliseFirst(related(related(row.misconceptions)?.skills)?.title ?? "Learning skill")} Â·
                                    seen {row.occurrence_count} times</p></div>) :
                        <p className="text-slate-600">No misconception evidence recorded for the selected learners and
                            curriculum scope.</p>}</div>
                </div>
                <div className="card"><h2 className="text-xl font-bold">Gamification overview</h2>
                    <div className="mt-4 grid grid-cols-2 gap-3"><Metric label="Badges awarded"
                                                                         value={String(badges?.filter(item => selectedLearners.has(item.learner_id)).length ?? 0)}/><Metric
                        label="Net coins issued"
                        value={String(coins?.filter(item => selectedLearners.has(item.learner_id)).reduce((sum, item) => sum + Number(item.amount), 0) ?? 0)}/>
                    </div>
                </div>
            </section>}
        </details>
        ...
        <details className="card mt-6">
            ...
            <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
                <div><p className="eyebrow">Reference</p><h2 className="mt-2 text-2xl font-bold">Complete Units /
                    Content Areas</h2></div>
                <Link className="link"
                      href={role === "administrator" ? "/teacher/content" : "/curriculum"}>{role === "administrator" ? "Open curriculum configuration" : "Preview learner curriculum"} â†’</Link>
            </div>
            <div className="mt-5 grid gap-5 lg:grid-cols-2">{courses?.map(course => {
                const units = (course.units ?? []) as {
                    id: string;
                    code: string;
                    title: string;
                    kind: string;
                    initial_teaching: boolean;
                    status: string
                }[];
                return <article className="rounded-2xl border border-slate-200 p-5" key={course.id}>
                    <p className="text-sm font-semibold text-teal-700">{course.qualification_type} Â· {course.qualification_level}</p>
                    <h3 className="mt-2 text-xl font-bold">{capitaliseFirst(course.title)}</h3>
                    <p className="mt-1 text-sm text-slate-500">{course.awarding_organisation} Â· {units.length} entries</p>
                    <ol className="mt-4 max-h-80 space-y-2 overflow-auto pr-2 text-sm">{units.sort((a, b) => Number(a.code) - Number(b.code)).map(unit =>
                        <li className="rounded-lg bg-slate-50 px-3 py-2" key={unit.id}>
                            <strong>{unit.code.match(/^\d+$/) ? `${unit.code}. ` : ""}{capitaliseFirst(unit.title)}</strong><span
                            className="ml-2 text-slate-500">{unit.kind.replaceAll("_", " ")}{unit.initial_teaching ? " Â· initial suggestion" : ""}</span>
                        </li>)}</ol>
                </article>;
            })}</div>
        </details>
...
function Metric({label, value, description}: { label: string; value: string; description?: string }) {
    return <div className="card"><p className="text-sm text-slate-500">{label}</p><p
        className="mt-2 text-2xl font-bold">{value}</p>{description &&
        <p className="mt-2 text-xs leading-5 text-slate-500">{description}</p>}</div>;
}

function journeyMilestoneLabel(value: string) {
    return value === "starting_point" ? "Starting Point" : value === "progress_check_1" ? "Progress Check 1" : value === "progress_check_2" ? "Progress Check 2" : value === "final" ? "Final / Summative" : "Weekly Learning";
}

function PriorityBadge({status}: { status: string }) {
    const values: Record<string, [string, string]> = {
        intervention_required: ["Intervention Required", "bg-red-100 text-red-900"],
        action_required: ["For your information", "bg-sky-50 text-sky-900"],
        catch_up_required: ["Catch-up Required", "bg-amber-100 text-amber-950"],
        on_track: ["On Track", "bg-emerald-100 text-emerald-900"],
        exceeding: ["Exceeding", "bg-blue-100 text-blue-900"]
    };
    const [label, colour] = values[status] ?? [status.replaceAll("_", " "), "bg-slate-100 text-slate-900"];
...
function PriorityCount({label, value, status}: { label: string; value: number; status: string }) {
    return <div className="rounded-xl border border-slate-200 p-4"><PriorityBadge status={status}/><p
        className="mt-3 text-3xl font-bold">{value}</p><p className="mt-1 text-xs text-slate-500">{label}</p></div>
}
...
function Signal({label, value, tone = "neutral"}: {
    ...
    return <div className={`rounded-xl p-4 ${colour}`}><p className="text-sm font-semibold">{label}</p><p
        className="mt-1 text-2xl font-bold">{value}</p></div>;
}
...
function FilterSelect({label, name, value, options}: {
    ...
    return <label className="grid gap-1 text-sm font-semibold">{label}<select className="input capitalize" name={name}
                                                                              defaultValue={value ?? ""}>
        <option value="">All</option>
        {options.map(option => <option value={option.id} key={option.id}>{option.title}</option>)}</select></label>;
}
...
function formatJourneyDate(value: string | null) {
    return value ? new Date(`${value}T12:00:00Z`).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "long",
        year: "numeric",
        timeZone: "UTC"
