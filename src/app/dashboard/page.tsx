...
async function AdministratorDashboard({role,filters }: {role:"administrator";filters:TeacherFilters }) {
  ...
  return <main className="shell py-10">
    ...
    <details className="card mt-6"><summary className="cursor-pointer text-lg font-bold">Earlier task alerts and setup suggestions</summary><p className="mt-3 text-slate-600">These alerts come from earlier targets and task allocations. Completing a new short lesson does not close an older target. Review the original reason before deciding whether action is still needed.</p>
    ...
    <details className="card mt-6"><summary className="cursor-pointer text-lg font-bold">Earlier assessments, targets and detailed reports</summary><p className="mt-2 text-sm text-slate-600">These totals and alerts use the earlier assessment and target system. They do not count the new short lessons shown above. Older alerts are retained for review and do not establish that current lessons were missed.</p>
    <section className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4" aria-label="Earlier workflow totals"><Metric label="Students" value={String(overview.students)}/><Metric label="Active enrolments" value={String(overview.activeEnrolments)}/><Metric label="Earlier assessments completed" value={String(overview.completedAssessments)}/><Metric label="Earlier group/student alerts" value={String(overview.needAttention)}/></section>
