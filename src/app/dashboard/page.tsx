...
async function StudentDashboard({ id, name }: { id: string; name: string }) {
  ...
  return <main className="shell py-10">
    ...
    {journeyPosition&&<StudentLearningPlan
      ...
      totalTeachingWeeks={Number(journeyPosition.total_teaching_weeks)}
      status={journeyPosition.position_status}
      pauseMessage={journeyPosition.position_status==="paused"?`${journeyPosition.pause_reason} does not count as a teaching week. Your journey resumes on ${formatJourneyDate(journeyPosition.next_teaching_on)}.`:undefined}
      currentTopic={currentJourneyWeek?{code:currentJourneyWeek.topicCode,title:capitaliseFirst(currentJourneyWeek.title),milestone:journeyMilestoneLabel(currentJourneyWeek.milestone),focus:capitaliseFirst(currentJourneyWeek.focus)}:undefined}
