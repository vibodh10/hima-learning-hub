...
export default async function AdminPage(){
  ...
  const[
    ...
    {data:classes},{data:retention},{data:badges},{data:deletionRequests},
    ...
    supabase.from("retention_settings").select("learner_evidence_years,archived_class_years,deletion_requires_approval").maybeSingle(),
  ...
  const requestedTeacherProfiles=requestedTeacherNames.flatMap(name=>{
    const profile=(profiles??[]).find(item=>item.display_name===name&&item.role==="teacher"&&!item.archived_at);
    if(!profile)return[];
    return[{name,id:profile.id}];
  });
  const requestedTeacherAccounts=await Promise.all(requestedTeacherProfiles.map(async profile=>{
    const{data}=await admin.auth.admin.getUserById(profile.id);
    return{name:profile.name,email:data.user?.email??null,status:data.user?"active" as const:"incomplete" as const};
  }));
  return <><AppHeader name={actor.display_name} role={actor.role}/><main className="shell py-10">
    ...
    <div className="mt-7"><p className="eyebrow">Administrator only</p><h1 className="mt-2 text-4xl font-bold">People and portal setup</h1><p className="mt-2 max-w-3xl text-slate-600">Teachers and students do not see this page. Everyday teaching happens from Groups; the controls below are only for occasional administrator work.</p></div>
    ...
    <section className="card mt-6"><p className="eyebrow">Configurable recognition</p><h2 className="mt-2 text-2xl font-bold">Badge criteria</h2><p className="mt-2 text-sm text-slate-600">Criteria changes are administrator-only.</p><div className="mt-5 grid gap-4 lg:grid-cols-2">{badges?.map(badge=><BadgeDefinitionForm badge={badge} key={badge.id}/>)}</div></section>
    </div></details>
  </main></>;
}
