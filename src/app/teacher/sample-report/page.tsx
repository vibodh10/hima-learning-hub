...
export default async function SampleReportPage({ searchParams }: { searchParams: Promise<{ stage?: string }> }) {
  ...
  return <><AppHeader name={actor.role === "teacher" ? "Hima" : actor.display_name} role={actor.role}/><main className="shell py-10">
    <Link className="link" href="/dashboard">← Hima’s dashboard</Link>
    <header className="mt-8 flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow">Sample learner report</p><h1 className="mt-2 text-4xl font-bold">What Hima sees during and after a path</h1><p className="mt-3 max-w-3xl text-slate-600">This fictional Unit 2 example shows the same evidence sections used by a real learner report. It does not change any learner data.</p></div><span className="rounded-full bg-violet-100 px-4 py-2 text-sm font-bold text-violet-900">Preview only</span></header>
    ...
    <section className="card mt-7"><p className="eyebrow">{sample.label}</p><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Fact label="Overall practical evidence" value={sample.overall}/><Fact label="Topics completed" value={sample.completed}/><Fact label="Current judgement" value={sample.status}/><Fact label="Teacher" value="Hima"/></div></section>
