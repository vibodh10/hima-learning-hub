...
export default async function ClassPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><AppHeader name={actor.display_name} role={actor.role}/><main className="shell py-10">
    <Link className="link inline-block" href={`/teacher/classes/${id}`}>← Current short-study records</Link>
    <aside className="card mt-6"><h2 className="text-xl font-bold">Earlier learning and administration records</h2><p className="mt-3">The scores and weekly alerts below belong to the earlier learning system. They do not measure the new daily self-study checks. Use current short-study records for today’s automatic targets.</p></aside>
    ...
    <section className="card mt-6"><h2 className="text-2xl font-bold">Daily self-study</h2><p className="mt-3">See short learning checks, recorded answers and automatic next targets for this group&apos;s current unit.</p><Link className="button mt-4" href={`/teacher/classes/${id}/self-study`}>Open self-study records →</Link></section>
