...
export default async function ClassPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: courses }, { data: units }, { data: periods }, {data:invitations}] = await Promise.all([
    ...
    supabase.from("student_invitations").select("id,email_normalized,display_name,status,invited_at,last_sent_at,accepted_at,cancelled_at,expired_at,send_count,last_detail_code,updated_at").eq("class_id",id).order("updated_at",{ascending:false}).limit(50),
  ]);
  ...
  return <><AppHeader name={actor.display_name} role={actor.role}/><main className="shell py-10">
    ...
    {selectedUnits.length>0&&<section className="card mt-6" aria-labelledby="unit-report-title">
      <p className="eyebrow">Evidence exports</p><h2 className="mt-2 text-2xl font-bold" id="unit-report-title">Unit evidence reports</h2>
    ...
    <section className={`card mt-6 ${learningReady?"border-teal-200 bg-teal-50":"border-amber-200 bg-amber-50"}`} aria-labelledby="automation-status-title"><p className="eyebrow">Automatic learning model</p><h2 className="mt-2 text-2xl font-bold" id="automation-status-title">{learningReady?"Invite students; the portal manages the routine workflow":"Do not invite students to this unit yet"}</h2><div className="mt-4 grid gap-2 text-sm text-slate-700"><p>✓ Unit starting point is the learner&apos;s first action.</p><p>✓ Adaptation changes support and challenge inside the class topic, not the group timetable.</p><p>✓ Objective questions give immediate system feedback and mistakes create further practice.</p><p>✓ Evidence and reports update from stored learner activity; empty evidence is shown honestly.</p><p>✓ No pathway-threshold, weekly-plan, reward, or adaptive-homework setup is required from the teacher.</p></div><p className="mt-4 text-xs text-slate-600">Open-ended and practical work that requires professional judgement is never presented as teacher feedback unless a teacher actually reviewed it.</p></section>

    {Boolean(invitations?.length)&&<section className="card mt-6" id="invitation-status" aria-labelledby="invitation-status-title">
    ...
    {journeyPosition ? <section className="card mt-6" aria-labelledby="journey-position-title">
      ...
      : selectedUnitIds.length>0&&classData.published
        ...
        : null}
    {studentIds.length>0&&overviewUnit&&(
      curriculumOverviewError
        ? <section className="card mt-8 border-red-200 bg-red-50" role="alert" aria-labelledby="curriculum-overview-error-title">
