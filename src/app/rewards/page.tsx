...
export default async function RewardsPage() {
  ...
  return <><AppHeader name={actor.display_name} role={actor.role}/><main className="shell py-10">
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <section className="card"><h2 className="text-lg font-bold">Coins to spend</h2><p className="mt-2 text-3xl font-bold">{postedBalance}</p><p className="mt-2 text-sm">Coins buy optional themes and other visual rewards. They are separate from XP.</p></section>
    </div>
    <details className="card mt-6"><summary className="cursor-pointer text-lg font-bold">My badges · {badges?.length??0} earned</summary><div className="mt-4 grid gap-3">{badges?.length?badges.map(badge=>{
      ...
      return <article className="rounded-xl border border-purple-200 p-4" key={badge.id}><h2 className="font-bold">★ {definition?.title??"Achievement badge"}</h2><p className="mt-1">{badge.reason||definition?.description}</p><p className="mt-2 text-sm text-slate-600">Earned {new Date(badge.awarded_at).toLocaleDateString("en-GB",{timeZone:"Europe/London"})}</p></article>;
