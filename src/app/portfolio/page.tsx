...
export default async function PortfolioPage() {
  ...
  return <><AppHeader name={actor.display_name} role={actor.role}/><main className="shell py-10">
    ...
    <section className="card mt-8 max-w-3xl border-blue-200 bg-blue-50"><p className="eyebrow">Your work is safe</p><h2 className="mt-2 text-2xl font-bold">{artifacts?.length??0} saved item{artifacts?.length===1?"":"s"}</h2><p className="mt-3 text-slate-700">You do not need to do anything on this page. Open the sections below only when you want to look back.</p></section>
    {comparisonKeys.length>0&&<details className="card mt-6"><summary className="cursor-pointer text-lg font-bold">Show my before-and-after progress</summary><div className="mt-5 grid gap-5">{comparisonKeys.map(key=>{const [unitCode,topicCode]=key.split(":");const topicWorks=(worksheets??[]).filter(item=>item.unit_code===unitCode&&item.topic_code===topicCode);const before=[...topicWorks].reverse().find(item=>item.evidence_stage==="before");const after=[...topicWorks].find(item=>item.evidence_stage==="after");return <article className="rounded-xl border border-slate-200 p-4" key={key}><p className="text-xs font-bold uppercase tracking-wide text-teal-700">Unit {unitCode} · {topicCode}</p><h3 className="mt-2 text-xl font-bold">Work over time</h3><div className="mt-4 grid gap-4 lg:grid-cols-2"><PortfolioStage title="Before teaching" worksheet={before}/><PortfolioStage title="After teaching" worksheet={after}/></div></article>;})}</div></details>}
    <details className="card mt-6"><summary className="cursor-pointer text-lg font-bold">Show all my saved work</summary><section className="mt-5 grid gap-4">
