    ...
    <section className="mt-12">
      ...
      <div className="mt-6 grid gap-4">{activities.map(activity => {
        ...
        return <article className="card flex flex-wrap items-center justify-between gap-5" key={activity.id}>
          <div><p className="text-sm font-bold text-teal-700">{activityLabel(activity)} · {activity.pathway}</p><h3 className="mt-1 text-xl font-bold">{activity.title}</h3><p className="mt-2 text-sm text-slate-600">{activity.instructions}</p>{activity.learning_stage==="mastery_check"&&!activity.assessment_kind&&<p className="mt-2 text-sm font-semibold text-amber-900">Independent check — no hints. This confirms whether you are ready to move ahead.</p>}<p className="mt-2 text-xs text-slate-500">{activity.activity_questions?.[0]?.count ?? 0} questions · about {activity.estimated_minutes} minutes {activity.required ? "· required" : "· optional"}</p></div>
          <div className="text-right"><span className={`mb-2 block rounded-full px-3 py-1 text-sm font-bold ${open?"bg-teal-50 text-teal-900":"bg-slate-100 text-slate-600"}`}>{state?.status_detail??(actor.role==="student"?"Available":"Test Mode preview")}</span>{open?<Link className="button" href={`/learn/${data.id}/activities/${activity.id}`}>{state?.state==="Completed"?"Review":"Open"} activity →</Link>:<span className="button-secondary cursor-not-allowed opacity-60">Locked</span>}</div>
        </article>;
      })}</div>
    </section>
