...
export default async function Unit14ExamPage({searchParams}:{searchParams:Promise<{classId?:string}>}){
 ...
 const attempts=group?await allStudyRows((from,to)=>{
  let query=client.from("exam_practice_attempts").select("id,learner_id,paper_id,activity,response,reflection,submitted_at,reviewed_at,user_profiles!exam_practice_attempts_learner_id_fkey(display_name)",{count:"exact"}).eq("class_id",group.id).in("paper_id",["guided",...unit14Papers.map(item=>item.id)]);
