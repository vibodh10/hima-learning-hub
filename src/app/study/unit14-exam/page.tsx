...
import {unit14Activities,unit14Checkpoint,unit14Papers,unit14Spec,unit14Catalogue} from "@/lib/unit14-exam";
...
export default async function Unit14ExamPage({searchParams}:{searchParams:Promise<{classId?:string}>}){
 ...
 return <div className="mini-study-surface"><header className="mini-study-header"><Link href={staff?"/dashboard":"/study"}>SCCB Learning</Link></header><main className="mini-study-main">
 <h1 className="text-3xl font-bold">Unit 14 · Ready for January</h1><p className="mt-3">IT Service Delivery · January 2027. Learn a little, practise it, then improve your answer.</p><p className="assignment-deadline">{unit14Checkpoint(studyDay(new Date()))}</p>
 ...
 <div className="mt-6"><Unit14Teaching/></div>
 <details className="mini-study-help mt-6"><summary>Choose from {unit14Papers.length} Pearson practice sets</summary><p>Resources from nine released past-task series and two sample sets. The June 2022 entry is Part B only. Some downloads are ZIP files: extract them on a computer and keep the matching instructions and templates together. Ask your teacher to help with missing files.</p><p>Use one new scenario at a time, then review it before starting another. Keep one unfamiliar paper for a full mock. These links open Pearson&apos;s documents; the portal does not claim to reproduce or mark them.</p>{unit14Papers.map(p=><article className="card mt-4" key={p.id}><h3 className="font-bold">{p.title}</h3>{p.note&&<p>{p.note}</p>}<ul className="mt-2 space-y-2">{p.links.map(l=><li key={l.url}><a className="link" href={l.url} target="_blank" rel="noreferrer">{l.title}</a></li>)}</ul></article>)}<p><a className="link" href={unit14Catalogue} target="_blank" rel="noreferrer">Pearson&apos;s full Unit 14 catalogue</a></p><p>Newer locked sets require teacher access. Hima or Lee can choose released materials for further practice.</p></details>
 <div id="practice">{group?<><p className="mt-6 font-bold">{group.name}</p><Unit14ExamPractice key={group.id} classId={group.id} staff={staff}/></>:<p className="card mt-6">No accessible Unit 14 group was found. Ask your teacher to check your enrolment.</p>}</div>
 <details className="mini-study-help mt-6"><summary>{staff?"Group attempts and self-reviews":"My saved attempts and self-reviews"} · {attempts.length}</summary><p>Submitted answers are preserved. Self-review means the learner compared and reflected; it is not a teacher-approved mark or an exam grade.</p>{attempts.map(a=>{const learner=Array.isArray(a.user_profiles)?a.user_profiles[0]:a.user_profiles;return <details key={a.id} className="card mt-4"><summary className="cursor-pointer font-bold">{staff?`${learner?.display_name??"Learner"} · `:""}{unit14Papers.find(p=>p.id===a.paper_id)?.title??"Original guided scenario"} · {unit14Activities[a.activity]} · {a.reviewed_at?"Self-review completed":"Answer submitted · review next"}</summary><p>{new Date(a.submitted_at).toLocaleDateString("en-GB",{timeZone:"Europe/London"})}</p><p className="whitespace-pre-wrap">{a.response}</p><details className="my-4"><summary>Review guidance</summary>{a.paper_id==="guided"?<><p>One strong approach, not an official answer:</p><p>{unit14Models[a.activity].model}</p></>:<p><a className="link" href={unit14Catalogue} target="_blank" rel="noreferrer">Find the matching Pearson marking grid and examiner report</a></p>}</details>{staff?<p className="whitespace-pre-wrap">{a.reflection??"No self-review yet."}</p>:<ExamReflectionForm id={a.id} initial={a.reflection??""}/>}</details>})}{!attempts.length&&<p>No written attempts submitted yet.</p>}</details>
