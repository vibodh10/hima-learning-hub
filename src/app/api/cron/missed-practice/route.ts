...
function londonNoon(day:string){
  // Noon UTC is safely inside the same Europe/London calendar day in both GMT and BST.
  return new Date(`${day}T12:00:00Z`);
}
function schoolDaysThisWeek(now=new Date()){
  const today=studyDay(now),start=practiceWeekStart(today),days:string[]=[];
  const cursor=londonNoon(start),end=londonNoon(today);
  while(cursor<=end){
    const weekday=cursor.getUTCDay();
    if(weekday>=1&&weekday<=5)days.push(cursor.toISOString().slice(0,10));
...
export async function POST(request:Request){
  ...
  let checked=0,daysChecked=0,missesCreated=0,learnerEmails=0,interventionsCreated=0,teacherEmails=0;
  const days=schoolDaysThisWeek();
  for(const learnerId of learners){
    checked+=1;
    for(const day of days){
      daysChecked+=1;
      // Re-run the same expectation engine for every school day in the current week.
      // This repairs a missed nightly job without inventing badges on weekends or closures.
      const expectation=await practiceExpectationForLearner(learnerId,londonNoon(day)).catch(()=>null);
  ...
  return NextResponse.json({ok:true,checked,daysChecked,missesCreated,learnerEmails,interventionsCreated,teacherEmails});
}
