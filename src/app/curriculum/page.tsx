...
export default async function CurriculumPage() {
  ...
  const workbookUnits = actor.role === "student"
    ? assignedUnits.map(unit => configuredByCode.get(unit.code)).filter((unit): unit is (typeof configuredUnits)[number] => Boolean(unit))
    : configuredUnits;
  ...
  return <main className="shell py-10">
    ...
    <p className="mt-4 max-w-4xl leading-7 text-slate-600">{actor.role === "student"
      ? "Only the programme and units assigned by your teacher are shown here. Open a ready unit to begin its starting-point assessment and learning activities."
    ...
    {actor.role === "student" && workbookUnits.length > 0 && <CourseWorkbookProgress units={workbookUnits} initialProgress={progress} storageKey={progressKeyFor(actor.id)}/>}
...
function TeacherPreviewList() {
  return <section className="mt-8 grid gap-5 md:grid-cols-2">{configuredUnits.map(unit => {
    ...
    return <Link className="card group hover:border-teal-400 hover:bg-teal-50" href={`/curriculum/units/${unit.code}`} key={unit.code}>
      ...
      <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold"><span className="rounded-full bg-slate-100 px-3 py-2">{meta.aims.length} learning aims</span><span className="rounded-full bg-slate-100 px-3 py-2">{unit.topics.length} topic lessons</span></div>
