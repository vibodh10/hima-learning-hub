...
export default async function UnitPage({ params }: { params: Promise<{ unitCode: string }> }) {
  ...
  if (!unit) notFound();
  const [initialProgress, actor] = await Promise.all([loadCurriculumProgress(), getSessionProfile()]);
  return <main className="shell py-10">
    ...
    <div className="mt-8"><UnitOverview unit={unit} initialProgress={initialProgress} storageKey={progressKeyFor(actor?.id ?? "guest")}/></div>
  </main>;
}
