-- Multi-unit learners can have older paused study rows from the starting-point-first flow.
-- Resume only the requested unit. Never unpause every stored row at once, because the
-- one-open-step index permits only one unpaused opened/review session per learner.
...
declare current_session public.mini_study_sessions; paused_session public.mini_study_sessions; new_id uuid; baseline_exists boolean;
begin
 ...
 -- A removed assignment invalidates unfinished work, but completed evidence stays intact.
 ...
 -- An already visible saved step always wins. This preserves the one-step-at-a-time flow.
 ...
 if lesson_value='it-prerequisites-10-v1' then
   ...
   if not baseline_exists and not exists(
     select 1 from public.mini_study_sessions
     where learner_id=learner_uuid and unit_id=unit_uuid
       and lesson_id='it-prerequisites-10-v1' and status='completed'
   ) then raise exception 'starting_point_required'; end if;

   -- A starting point may have paused an older lesson. Resume only this unit's
   -- saved row rather than unpausing rows belonging to other Thursday-group units.
