...
begin
  ...
  if length(trim(title_value))<3
    ...
    or (academic_period_uuid is not null and not exists(
      select 1 from public.academic_periods where id=academic_period_uuid
        and academic_year_id=academic_year_uuid and archived_at is null
  ...
  if event_uuid is null then
    ...
    where id=event_uuid and academic_year_id in (
