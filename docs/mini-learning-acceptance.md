...
## Fast release preparation after the user's usage warning

- User requested finishing quickly and avoiding further redesign. No new visual redesign is being undertaken.
- Added a private teacher CSV download of the current short-study evidence, including retained full baselines, first responses, separate recap/new scores and automatic targets. Shared the exact same paginated evidence loader with the on-screen report; query errors, truncation or changing counts produce an error rather than a misleading partial report. Names are protected against spreadsheet formula interpretation.
- Fourteen focused export/permission/pagination/report tests passed; lint and production build passed. The authenticated teacher page successfully rendered the new download link and existing saved learner evidence.
- A read-only live assignment check found eight published groups with current units 2, 4, 6, 11 and 19. Added four original introductory self-study steps each for Units 2, 4 and 19, with basic starting-point instruments, matching, examples and optional analysis/evaluation support. These supplement teaching; they are not full qualification coverage. All currently active unit codes now have mini content. Further non-active catalogue units are not claimed ready.
- Expanded pack checks to all eight available mini packs (2, 4, 5, 6, 9, 11, 16, 19): 18 content/rule tests passed and the current production build passed. Final full regression run and deployment pending at this entry.
- Teacher settings retest saved the QA group's unchanged Units 6/11 and current Unit 6 successfully. Closed the QA joining link through the teacher UI; it confirmed that no one else can use it. QA identities/group remain labelled for remaining checks; real records unchanged.

## Production release (2026-09-08, in progress)

- Exact application revision: `f8d4821a863140c5995982193af39a07f131dcca`.
- Final full regression: **433 passed, 3 skipped**, 107 passing files, 191 seconds. Current production build passed. No application code changed after these checks.
- Published that revision by fast-forwarding the existing production branch from 75568bf. Railway deployment `510d58e4-cabd-466d-9bab-9fb65feeb684` reached SUCCESS. The live `/api/release` endpoint returned the exact revision and production environment. Signed-out report download returned 401 with private/no-store caching. Production login rendered correctly in the browser.
- This release is not a claim of exhaustive goal completion: final live mobile/200% and theme checks, broader edge cases and retirement of the labelled QA entities remain to verify. Non-active catalogue units without mini packs are not claimed ready.
- Post-deployment signed-in QA teacher check passed: only its own group appeared, and its expanded report retained baseline 3/4, latest new check 2/2, recap 1/1, two completed daily steps and the current automatic target. The short-study download link was present.
- Post-deployment signed-in QA student check passed: redirected to `/study`, retained the completed daily step and 20-XP receipt, no repeat Start or unit picker, and restored purple/magenta/lavender styling. Visually checked the current 551-pixel-wide browser panel without horizontal overflow. Signed out the QA account and left the production login page open for the user. This is not a substitute for the remaining explicit 200%/phone checks.
